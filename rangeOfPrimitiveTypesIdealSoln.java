package HackerRankLeetCodeQuestions;


import java.util.*;
import java.io.*;
import java.lang.Math;



class Solution{
    public static void main(String []argh)
    {

        Scanner sc = new Scanner(System.in);
        int t=sc.nextInt();

        for(int i=0;i<t;i++)
        {

            try
            {
                long x=sc.nextLong();
                System.out.println(x+" can be fitted in:");
                if(x>=-128 && x<=127)System.out.println("* byte");
                
                long negShort = Short.MIN_VALUE;
                long posShort = Short.MAX_VALUE;
                if( (x >= negShort) && (x <= posShort))System.out.println("* short");
                
                long negInt = Integer.MIN_VALUE;
                long posInt = Integer.MAX_VALUE;
                if( (x >= negInt) && (x <= posInt))System.out.println("* int");
                
                long negLong = Long.MIN_VALUE;        
                long posLong = Long.MAX_VALUE;
                if( (x >= negLong) &&  (x <= posLong))System.out.println("* long");
                
                // else System.out.println("n can't be fittedd anywhere.");
                //Complete the code
            }
            catch(Exception e)
            {
                System.out.println(sc.next()+" can't be fitted anywhere.");
            }

        }
    }
}





